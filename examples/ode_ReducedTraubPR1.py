...
class ODEFunc(nn.Module):
    ...
    def __init__(self):
        ...
        #Make initial conditions optimizable
        self.h0=torch.nn.Parameter(1.0*torch.tensor(0,dtype=torch.float))
        self.register_parameter('h0', self.h0)
        self.n0=torch.nn.Parameter(1.0*torch.tensor(0,dtype=torch.float))
        self.register_parameter('n0', self.n0)

        # self.h0_flip=torch.nn.Parameter(1.0*torch.tensor(0,dtype=torch.float))
        # self.register_parameter('h0_flip', self.h0_flip)
        # self.n0_flip=torch.nn.Parameter(1.0*torch.tensor(0,dtype=torch.float))
        # self.register_parameter('n0_flip', self.n0_flip)

        self.h0_1=torch.nn.Parameter(1.0*torch.tensor(0,dtype=torch.float))
        self.register_parameter('h0_1', self.h0_1)
        self.n0_1=torch.nn.Parameter(1.0*torch.tensor(0,dtype=torch.float))
        self.register_parameter('n0_1', self.n0_1)

        self.h0_2=torch.nn.Parameter(1.0*torch.tensor(0,dtype=torch.float))
        self.register_parameter('h0_2', self.h0_2)
        self.n0_2=torch.nn.Parameter(1.0*torch.tensor(0,dtype=torch.float))
        self.register_parameter('n0_2', self.n0_2)
...
if __name__ == '__main__':
    ...
    #t_sub_flip = torch.from_numpy(np.flip(t_sub.numpy()))
    # t_sub_flip = torch.zeros_like(t_sub)
    # true_y_sub_flip = torch.zeros_like(true_y_sub)
    # for i in range(t_sub.shape[0]):
    #     t_sub_flip[i]=t_sub[t_sub.shape[0]-i-1]
    #     true_y_sub_flip[i] = true_y_sub[t_sub.shape[0] - i - 1]
    # print(t_sub_flip)
    # print(true_y_sub_flip)
    print('Fitting interval {} to {}'.format(i1,i2))
    ...
    for itr in range(1, args.niters + 1):
        ...
        # imid = int((i2-i1)/2)
        # # imid2 = int(3*(i2-i1)/4)
        # init_state_1 = torch.tensor([[0,0,0]], dtype=torch.float)
        # init_state_1[0,0] = true_y_sub[imid,0,0]
        # init_state_1[0,1] = func.h0_1
        # init_state_1[0,2] = func.n0_1
        # print('Initial state 1:', init_state_1)

        # init_state_2 = torch.tensor([[0, 0, 0]], dtype=torch.float)
        # init_state_2[0, 0] = true_y_sub[imid2, 0, 0]
        # init_state_2[0, 1] = func.h0_2
        # init_state_2[0, 2] = func.n0_2
        # print('Initial state 2:', init_state_2)

        # init_state_flip = torch.tensor([[0,0,0]], dtype=torch.float)
        # init_state_flip[0,0] = true_y_sub[-1,0,0]
        # init_state_flip[0,1] = func.h0_flip
