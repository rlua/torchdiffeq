...
parser.add_argument('--position_noise_stddev', type=float, default=0) #RCL added new option. Try 0.01
parser.add_argument('--velocity_noise_stddev', type=float, default=0) #RCL added new option. Try 0.05
...
class ODEFunc(nn.Module):
    ...
    def __init__(self, randomseed_initialparams):
        ...
        self.net = nn.Sequential(
            nn.Linear(N2_, args.nhidden),
            #nn.Tanh(),
            nn.ReLU(),
            #nn.Linear(args.nhidden, args.nhidden),
            #nn.ReLU(),
            nn.Linear(args.nhidden, N2_),
        )

        torch.manual_seed(randomseed_initialparams) #RCL
...
if __name__ == '__main__':
    ...
    for itr in range(1, args.niters + 1):
        ...
        if itr % args.test_freq == 0:
            with torch.no_grad():
                ...
                print('Iter {:04d} | Total Loss {:.6f} | Train Loss Before Optimizer Update {:.6f}'.format(itr, totallossval, trainlossval))
    ...
    print('Seed:',args.randomseed_initialparams,'Loss:', min_loss, 'Iter:', best_iter)